...
const appCard = (goApp: string): JSX.Element => (
  <a
    ...
    className="w-full h-full cursor-sewingHS flex items-center justify-center"
    style={{
      color: "#cdd2d8",
      fontSize: 9,
...
const renderCard = (p: any, lbl: any): JSX.Element => (
  <div className="w-full h-full flex flex-row" style={{ padding: 8, gap: 8 }}>
    ...
    >
      ...
      <div
        style={{ fontSize: 7, lineHeight: 1.25, opacity: 0.78, ...clamp(2) }}
      >
        {p.desc}
      </div>
      {p.hardware && (
...
const RingFunnel = ({ dict }: { dict?: any }): JSX.Element => {
  ...
  const goApp: string = dict?.common?.goApp;
    ...
    <>
      ...
      >
        {CHAIN.slice(0, 8).map((c, i) => {
          const p = i < 6 ? phases[i] : i === 7 ? phases[6] : undefined;
          if (!p) return null;
          const left = i < 6 ? i % 2 === 0 : true;
          const svgLeft = left
